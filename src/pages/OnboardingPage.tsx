    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white" dir={dir}>
      <div className="max-w-3xl mx-auto px-5 py-10 md:py-14">
        <div className="flex items-center gap-2 mb-8">
          {[1, 2, 3].map((s) => (
            <div key={s} className={`h-1.5 flex-1 rounded-full transition-colors ${s <= step ? "bg-[#2ECC71]" : "bg-slate-200"}`} />
          ))}
        </div>

        <div className="flex items-center justify-between mb-2">
          <span className="inline-flex items-center gap-1.5 text-xs font-bold text-[#2ECC71] uppercase tracking-wider">
            <Sparkles className="w-3.5 h-3.5" /> {t("onb.step", { current: step, total: 3 })}
          </span>
          <button type="button" onClick={() => finish(true)} disabled={saving} className="text-xs font-semibold text-slate-500 hover:text-slate-700">
            {t("onb.skip")}
          </button>
        </div>
        ...
        {step === 1 && (
          <>
            <h1 className="text-3xl md:text-4xl font-extrabold text-[#2C3E50] tracking-tight">{t("onb.s1.title")}</h1>
            <p className="mt-2 text-[#2C3E50]/65">{t("onb.s1.subtitle")}</p>
            ...
            <div className="mt-6 relative">
              ...
              <input
                ...
                placeholder={t("onb.s1.search")}
                className={`w-full ${dir === "rtl" ? "pr-9 pl-3" : "pl-9 pr-3"} py-3 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:border-[#2ECC71]`}
              />
            </div>
              ...
              {filteredTeams.map((team) => {
                ...
                return (
                  <button
                    key={team.id}
                    type="button"
                    onClick={() => toggle(teams, setTeams, team.id)}
                    className={`relative text-left rounded-xl border p-3 transition-all ${
                      sel ? "border-[#2ECC71] bg-[#2ECC71]/5 ring-2 ring-[#2ECC71]/20" : "border-slate-200 hover:border-slate-300 bg-white"
                    }`}
                  >
                    {sel && (
                      <span className={`absolute top-2 ${dir === "rtl" ? "left-2" : "right-2"} w-5 h-5 rounded-full bg-[#2ECC71] flex items-center justify-center`}>
                        <Check className="w-3 h-3 text-white" />
                      </span>
                    )}
                    <div className="text-2xl mb-1.5">{team.emoji}</div>
                    <p className="font-bold text-sm text-[#2C3E50] leading-tight">{team.name}</p>
                    <p className="text-[11px] text-slate-500 mt-0.5">{team.league}</p>
                  </button>
                );
              })}
            </div>
        ...
        {step === 2 && (
          <>
            <h1 className="text-3xl md:text-4xl font-extrabold text-[#2C3E50] tracking-tight">{t("onb.s2.title")}</h1>
            <p className="mt-2 text-[#2C3E50]/65">{t("onb.s2.subtitle")}</p>
              ...
              {LEAGUES.map((l) => {
                ...
                return (
                  ...
                  >
                    ...
                    {sel && (
                      <span className="ms-auto w-5 h-5 rounded-full bg-[#2ECC71] flex items-center justify-center">
                        <Check className="w-3 h-3 text-white" />
                      </span>
                    )}
                  </button>
        ...
        {step === 3 && (
          <>
            <h1 className="text-3xl md:text-4xl font-extrabold text-[#2C3E50] tracking-tight">{t("onb.s3.title")}</h1>
            <p className="mt-2 text-[#2C3E50]/65">{t("onb.s3.subtitle")}</p>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="font-bold text-[#2C3E50]">{t(`onb.intent.${i.id}.title`)}</p>
                      <p className="text-xs text-[#2C3E50]/60 mt-0.5">{t(`onb.intent.${i.id}.desc`)}</p>
                    </div>
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center ${sel ? "bg-[#2ECC71]" : "border-2 border-slate-200"}`}>
        ...
        <div className="mt-10 flex items-center justify-between">
          {step > 1 ? (
            <button type="button" onClick={back} className="text-sm font-semibold text-slate-600 hover:text-slate-800">
              {t("onb.back")}
            </button>
          ) : <span />}

          {step < 3 ? (
            <button type="button" onClick={next} className="inline-flex items-center gap-2 rounded-xl bg-[#2C3E50] hover:bg-[#1f2d3a] text-white px-6 py-3 font-bold shadow-md transition-colors">
              {t("onb.continue")} <ChevronRight className={`w-4 h-4 ${dir === "rtl" ? "rotate-180" : ""}`} />
            </button>
          ) : (
            <button type="button" onClick={() => finish(false)} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[#2ECC71] hover:bg-[#27ae60] text-white px-6 py-3 font-bold shadow-lg shadow-[#2ECC71]/30 transition-colors disabled:opacity-60">
              {saving && <Loader2 className="w-4 h-4 animate-spin" />} {t("onb.finish")}
            </button>
          )}
        </div>
      </div>
